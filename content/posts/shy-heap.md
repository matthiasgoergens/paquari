...
these are exactly [nested
matroids](https://www.math.lsu.edu/~oxley/LaminarMatroids_revised.pdf):
appending an insertion adds a coloop, appending a deletion truncates,
and Fife and Oxley's Theorem 1.4 characterises precisely the class
generated by those two operations. Laminar matroids are a strictly
broader class.

Parts of the literature call the same isomorphism types *freedom*,
*generalised Catalan*, *shifted*, or *Schubert* matroids. The last name
is convention-sensitive. Some authors mean an ordered matroid tied to a
particular Gale order and Schubert cell — a definition explicitly [not
closed under relabelling](https://link.springer.com/article/10.1007/s00493-024-00085-4)
— while others close the class under matroid isomorphism. Those
conventions agree after forgetting the chosen order, but they are not
the same labelled object. I therefore keep *heap matroid* for this
trace presentation while identifying its underlying abstract matroid as
nested. Running a min-heap is finding the maximum-weight basis. The
family is closed under [matroid
